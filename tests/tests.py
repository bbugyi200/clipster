...
class ClientTestCase(unittest.TestCase):
    ...
    def test_client_update(self, mock_socket):
        ...
        self.assertTrue(
            mock.call.connect(os.path.join(self.data_dir, socket_file))
            in sock.mock_calls
        )
        self.assertTrue(
            mock.call.sendall(
    ...
    def test_client_output(self, mock_socket):
        ...
        sock.recv.side_effect = [
            json.dumps(self.history[board]).encode("utf-8"),
            "",
        ]
        ...
        self.assertTrue(
            mock.call.sendall(
                "{}:{}:{}".format(client_action, board, count).encode("utf-8")
            )
            in sock.mock_calls
        )
...
class DaemonTestCase(unittest.TestCase):
    ...
    def test_read_history_file(self):
        ...
        with mock.patch.object(
            builtins,
            "open",
            mock.mock_open(read_data=json.dumps(self.history)),
        ) as mock_file:
    ...
    def test_process_msg_invalid(self, mock_logging):
        ...
        mock_logging.assert_called_with(
            "Invalid message received via socket: %s", "INVALID MESSAGE"
        )
    ...
    def test_process_msg_delete_pattern_match(self):
        ...
        board_length = len(self.history[board])
        self.daemon.client_msgs = {
            1: "{}:{}:{}:{}".format(action, board, count, pattern)
        }
        self.daemon.process_msg(conn)
        # Board should be one item less
        self.assertEqual(board_length - len(self.history[board]), 1)
    ...
    def test_get_list_from_option_string(self):
        ...
        self.assertEqual(
            clipster.get_list_from_option_string("thunar,chromium,catfish"),
            ["thunar", "chromium", "catfish"],
        )
    ...
    def test_filtered_window_classes(self, mock_class):
        ...
        self.daemon.p_id = self.daemon.primary.connect(
            "owner-change", self.daemon.owner_change
        )
        self.daemon.c_id = self.daemon.clipboard.connect(
            "owner-change", self.daemon.owner_change
        )

        self.daemon.blacklist_classes = [
            "thunar",
            "chromium",
            "kate",
            "catfish",
